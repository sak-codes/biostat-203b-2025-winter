...
ggplot() +
  ...
  # Procedures
  geom_point(data = procedures_data_1, 
             aes(x = chartdate, y = "Procedure", shape = long_title), size = 5, position = position_nudge(y = -0.2)) +
...
![](labevents_tble.png)
```{r}
# Load ICU stays and lab events
icustays_tble <- read_csv("~/mimic/icu/icustays.csv.gz") %>% as_tibble()
labevents_tble <- open_dataset("~/repos/biostat-203b-2025-winter/hw3/labevents_pq.parque/part-0.parquet", format = "parquet")


```
```{r}
list.files("~/repos/biostat-203b-2025-winter/hw3/labevents_pq.parque", pattern = "*.parquet", full.names = TRUE)
...
# Lab items of interest
lab_items <- c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)

# Filter relevant lab measurements and join with ICU stays
labevents_tble_filtered <- labevents_tble %>%
  ...
  select(subject_id, hadm_id, bicarbonate, chloride, creatinine, glucose, potassium, sodium, hematocrit, wbc)

# Join with ICU stays to include stay_id
labevents_tble_final <- icustays_tble %>%
  select(subject_id, hadm_id, stay_id) %>%
  inner_join(labevents_tble_filtered, by = c("subject_id", "hadm_id"))

# Preview the final lab events table
print(labevents_tble_final)
s
...
```{r}
# Example for summarizing length of stay by demographic variables
mimic_icu_cohort %>%
  group_by(gender, race, insurance, marital_status) %>%
  summarize(
