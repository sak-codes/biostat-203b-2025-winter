...
# Step 3: Filter and join the data locally in R (after collecting both datasets)
...
preprocessed_data <- mimic_icu_cohort %>%
  ...
  # i) Lump infrequent levels into "Other" for specific categorical variables
   mutate(
    # Convert to factor and replace NAs with "Other"
    #race = fct_explicit_na(race, na_level = "Unknown"),
    #last_careunit = last_careunit %>% as_factor() %>% fct_explicit_na(na_level = "Unknown"),
    #admission_type = admission_type %>% as_factor() %>% fct_explicit_na(na_level = "Unknown"),
    #admission_location = admission_location %>% as_factor() %>% fct_explicit_na(na_level = "Unknown"),
    #discharge_location = discharge_location %>% as_factor() ,
     discharge_location = discharge_location %>%as_factor() %>%fct_drop(only = ""),
    
    # Lump infrequent levels into "Other"
    first_careunit = fct_lump_n(first_careunit, n = 4, other_level = "Other"),
    # first_careunit = first_careunit %>% as_factor() %>% fct_lump_n(n = 4, other_level = "Other") %>% fct_explicit_na(na_level = "Unknown"),
    ...
    race = fct_collapse(race,
                        ...
                        WHITE = c("WHITE - BRAZILIAN", "WHITE - EASTERN EUROPEAN", 
                                  "WHITE - OTHER EUROPEAN", "WHITE - RUSSIAN"),
                        Other = c("AMERICAN INDIAN", "PATIENT DECLINED TO ANSWER", 
                                  "PORTUGUESE", "SOUTH AMERICAN", "UNABLE TO OBTAIN", "UNKNOWN", 
                                  "OTHER", "AMERICAN INDIAN/ALASKA NATIVE", "MULTIPLE RACE/ETHNICITY",
                                  "NATIVE HAWAIIAN OR OTHER PACIFIC ISLANDER"),
                        #Unknown = c("UNKNOWN", "UNKNOWN RACE")
    ),
  ...
  # -glucose_group, -glucose_bin, -sodium_bin, -potassium_bin, ,-creatinine_bin,-heart_rate_bin,-systolic_bp_bin,-diastolic_bp_bin,
   #      -body_temp_bin,-resp_rate_bin,-wbc_bin
  # iv) Remove unnecessary variables
  select(-subject_id, -stay_id, -hadm_id, -intime, -outtime, -admittime, -dischtime, -deathtime, 
         -admit_provider_id,-edregtime,-dod,-edouttime,-anchor_year_group,-anchor_age,-anchor_year
         )
...
summary_table <- tbl_summary(
  ...
  statistic = list(all_categorical() ~ "{n} ({p}%)", all_continuous() ~ "{median} ({p25}),{p75}"),  # Adjust statistics for categorical/continuous variables
